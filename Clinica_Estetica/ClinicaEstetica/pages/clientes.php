...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['acao'] === 'cadastrar') {
    ...
    if ($nome && $telefone) {
        $pdo->prepare("INSERT INTO cliente (nome, telefone) VALUES (?, ?)")->execute([$nome, $telefone]);
...
if (isset($_GET['excluir'])) {
    $pdo->prepare("DELETE FROM cliente WHERE id_cliente = ?")->execute([$_GET['excluir']]);
...
<div class="topbar"><h5 class="mb-0 text-muted">Gerenciar Clientes</h5></div>
<div class="conteudo">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <h6 class="mb-3"><i class="bi bi-person-plus me-2 text-danger"></i>Novo Cliente</h6>
            <form method="POST">
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-body p-4">
            <table class="table table-hover align-middle">
                ...
                <tbody>
                <?php foreach ($pdo->query("SELECT * FROM cliente ORDER BY nome")->fetchAll(PDO::FETCH_ASSOC) as $c): ?>
                    <tr>
                        <td><?= $c['id_cliente'] ?></td>
                        <td><?= htmlspecialchars($c['nome']) ?></td>
                        <td><?= htmlspecialchars($c['telefone']) ?></td>
                        <td>
                            <a href="?excluir=<?= $c['id_cliente'] ?>" class="btn btn-sm btn-outline-danger"
                               onclick="return confirmarExclusao('Excluir este cliente?')">
